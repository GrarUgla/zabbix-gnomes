...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                 description='Tries to find items configured for the specified Zabbix host.', epilog="""
...
group.add_argument('-e', '--extended',
                   help='Returns itemid, value_type, status, state, key, and name separated by ":". See https://www.zabbix.com/documentation/2.2/manual/api/reference/item/object for more information',
                   action='store_true')
        ...
        if args.extended:
            ...
            for item in items:
                print(format(item["itemid"]) + ":" + format(item["value_type"]) + ":" + format(
                    item["status"]) + ":" + format(item["state"]) + ":" + format(item["key_"]) + ":" + format(
                    item["name"]))
        else:
            if args.numeric:
                # print ids
