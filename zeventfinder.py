...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                 description='Finds Zabbix events and prints them in a syslog like format. If the termcolor module is found, it is used to generate colored output.',
                                 epilog="""
...
parser.add_argument('-n', '--numeric', help='Use numeric ids instead of names, applies to -H and -G',
                    action='store_true')
parser.add_argument('-L', '--limit', help='Limit the number of returned lines, default is 100. Set to 0 to disable.',
...
parser.add_argument('-t', '--time-period', help='Timeperiod in seconds, default is one week. Set to 0 to disable.',
    ...
    while True:
        ...
        if events:
            if args.ids:
                ...
                triggers = zapi.trigger.get(triggerids=triggerids, output='extend', expandDescription=1, preservekeys=1,
                                            expandComment=1, selectHosts='extend')
                for event in sorted(events):
                    eventid = event['eventid']
                    ...
                    print
                    "%s %s: %s [%s] %s [%s](%s|%s)" % (
                    etime, hostname, state, eventid, trigger, triggerid, severity, acknowledged)
                    if args.follow:
                        sys.stdout.flush()
