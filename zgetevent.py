...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                 description='Gets event details for Zabbix events. If the termcolor module is found, it is used to generate colored output.',
                                 epilog="""
...
parser.add_argument('-L', '--alerts', help='Display alert actions (ignored when using short format)',
                    action='store_true')
...
if events:
    ...
    for event in events:
        ...
        if args.short:
            ...
            print
            "%s %s: %s [%s] %s [%s](%s|%s)" % (
            time, hostname, state, eventid, trigger, triggerid, severity, acknowledged)
        else:
            if acked == True:
