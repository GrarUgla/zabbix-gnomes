...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                 description='Queries inventory data for the specified host(s) or hostgroup(s).',
                                 epilog="""
...
parser.add_argument('-i', '--with-inventory', help='Only return data for hosts that have inventory',
                    action='store_true')
...
if args.all_hosts:
    ...
    if args.hostgroups:
        ...
        if args.numeric:
            ...
            if args.monitored:
                if args.with_inventory:
                    hlookup = zapi.host.get(output=['hostid'], monitored_hosts=True, withInventory=True,
                                            filter=({'host': args.hostnames}))
                else:
